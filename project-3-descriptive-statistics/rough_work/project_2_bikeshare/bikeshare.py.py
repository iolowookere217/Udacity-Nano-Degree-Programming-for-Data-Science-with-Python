...
def user_stats(df):
    ...
    # display the top 5 and buttom 5 raw data
    p = df.head(5)
    q = df.tail(5)
    r = pd.concat([p,q],ignore_index=False)
    while True:
        view_Data = input("Would you like to see the top and bottom 5 raw data? Type 'Yes' or 'No'.").lower()
        if view_Data == "yes":
            print(r)
